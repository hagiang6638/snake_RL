...
class Agent:
    ...
    def get_state(self, game):
        head = game.snake[0]
        d = 20
        point_l = Point(head.x - d, head.y)
        point_r = Point(head.x + d, head.y)
        point_u = Point(head.x, head.y - d)
        point_d = Point(head.x, head.y + d)
    ...
    def train_short_memory(self, state, action, reward, next_state, done):
        loss = self._train_step(state, action, reward, next_state, done)
        return loss
